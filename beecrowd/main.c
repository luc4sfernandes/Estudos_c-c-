...
// Define _DEFAULT_SOURCE para habilitar algumas extensões POSIX e GNU,
// incluindo funções como usleep e strcasecmp.
#define _DEFAULT_SOURCE
#include <stdio.h>
#include <string.h>  // Para strlen, strncpy, strcpy
#include <stdlib.h>  // Para malloc, free
#include <ctype.h>   // Para tolower
#include <time.h>    // Para clock, CLOCKS_PER_SEC

// POSIX headers para funções como usleep e strcasecmp.
// Para compatibilidade total com C99, pode ser necessário definir
// _DEFAULT_SOURCE ou _POSIX_C_SOURCE antes de incluir estes.
#include <unistd.h>  // Para usleep (em sistemas não-Windows)
#include <strings.h> // Para strcasecmp (em sistemas POSIX, ex: ordenarVetor)

#ifdef _WIN32
#include <windows.h>
#else
// Para sistemas não-Windows, Sleep é um wrapper para usleep
...
// Definições de tamanhos para as listas de itens (para evitar números mágicos)
#define TAM_ARMAS_CURTAS 5
#define TAM_ARMAS_LONGAS 5
#define TAM_CONSUMIVEIS 4

const char* submetralhadora[TAM_ARMAS_CURTAS]    = { "MP40", "P90", "UMP", "Thompson", "MP5" };
const char* rifles_assalto[TAM_ARMAS_LONGAS]     = { "AK-47", "M4A1", "SCAR", "FAMAS", "XM8" };
const char* rifles_de_precisao[TAM_ARMAS_LONGAS] = { "AWM", "Kar98K", "M24", "Treatment Sniper", "VSK94" };
const char* escopetas[TAM_ARMAS_LONGAS]          = { "M1887", "M1014", "SPAS12", "Charge Buster", "MAG-7" };
const char* pistolas[TAM_ARMAS_LONGAS]           = { "Desert Eagle", "M500", "G18", "USP", "Treatment Gun" };
const char* armas_brancas[TAM_ARMAS_LONGAS]      = { "Katana", "Panela", "Bastão", "Machado", "Foice" };
const char* consumiveis[TAM_CONSUMIVEIS]        = { "Kit Médico", "Inalador", "Cogumelos", "Kit de Reparar" };
...
/* Protótipos para funções utilitárias gerais (movidas para cima) */
void limpar_buffer();
void trim_newline(char *s); // Declaração da função trim_newline
int str_casecmp_c(const char *a, const char *b); // Declaração da função str_casecmp_c

/* Protótipos - Mochila */
void listar_opcoes(const char* lista[], int tamanho);
int nome_igual_ci(const char *a, const char *b); // nome_igual_ci será refatorada para usar str_casecmp_c
...
/* Lista */
void inserirItemLista(No** cabeça, No** cauda); // Adicionado ponteiro para cauda
...
void liberarLista(No** cabeça); // liberarLista não precisa da cauda, pois percorre do início
...
/* Funções utilitárias gerais (definições movidas para cima) */
...
int str_casecmp_c(const char *a, const char *b) {
    while (*a && *b) {
...
int nome_igual_ci(const char *a, const char *b) {
    // Usando a função str_casecmp_c para consistência
    return str_casecmp_c(a, b) == 0;
}

/* Encapsula seleção de item por categoria */
void escolher_item_por_categoria(int opcao, char* destino_nome, char* destino_tipo) {
    int escolha = -1;
    char buffer_nome[30]; // Buffer temporário para nome
    switch (opcao) {
        case 0:
            strncpy(destino_tipo, "Item Extra", 19); destino_tipo[19] = '\0';
            printf("Digite o nome do item: ");
            if (fgets(buffer_nome, sizeof(buffer_nome), stdin)) {
                trim_newline(buffer_nome);
                strncpy(destino_nome, buffer_nome, 29);
                destino_nome[29] = '\0';
            } else {
                destino_nome[0] = '\0'; // Em caso de erro na leitura
            }
            return;
        case 1:
            strcpy(destino_tipo, "Submetralhadora");
            do {
                listar_opcoes(submetralhadora, TAM_ARMAS_CURTAS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_ARMAS_CURTAS) printf("Opção inválida! Tente novamente.\n");
            } while (escolha < 0 || escolha >= TAM_ARMAS_CURTAS);
        ...
        case 2:
            ...
            do {
                listar_opcoes(rifles_assalto, TAM_ARMAS_LONGAS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_ARMAS_LONGAS) printf("Opção inválida! Tente novamente.\n");
            } while (escolha < 0 || escolha >= TAM_ARMAS_LONGAS);
        ...
        case 3:
            ...
            do {
                listar_opcoes(rifles_de_precisao, TAM_ARMAS_LONGAS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_ARMAS_LONGAS) printf("Opção inválida! Tente novamente.\n");
        ...
        case 4:
            ...
            do {
                listar_opcoes(escopetas, TAM_ARMAS_LONGAS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_ARMAS_LONGAS) printf("Opção inválida! Tente novamente.\n");
        ...
        case 5:
            ...
            do {
                listar_opcoes(pistolas, TAM_ARMAS_LONGAS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_ARMAS_LONGAS) printf("Opção inválida! Tente novamente.\n");
        ...
        case 6:
            ...
            do {
                listar_opcoes(armas_brancas, TAM_ARMAS_LONGAS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_ARMAS_LONGAS) printf("Opção inválida! Tente novamente.\n");
        ...
        case 7:
            ...
            do {
                listar_opcoes(consumiveis, TAM_CONSUMIVEIS);
                printf("Escolha o número do item: ");
                if (scanf("%d", &escolha) != 1) escolha = -1;
                limpar_buffer();
                if (escolha < 0 || escolha >= TAM_CONSUMIVEIS) printf("Opção inválida! Tente novamente.\n");
            } while (escolha < 0 || escolha >= TAM_CONSUMIVEIS);
...
void ordenarVetor(Item* vetor, int num) {
    ...
    for (i = 0; i < num - 1; i++) {
        for (j = 0; j < num - 1 - i; j++) {
            if (str_casecmp_c(vetor[j].nome, vetor[j+1].nome) > 0) { // Usando str_casecmp_c
                tmp = vetor[j];
                vetor[j] = vetor[j+1];
                vetor[j+1] = tmp;
            }
...
int buscarBinariaVetor(Item* vetor, int num, const char* nome) {
    ...
    while (left <= right) {
        ...
        int cmp = str_casecmp_c(vetor[mid].nome, nome); // Usando str_casecmp_c para consistência
        if (cmp == 0) return mid;
        if (cmp < 0) left = mid + 1;
        else right = mid - 1;
    }
...
void inserirItemLista(No** cabeça, No** cauda) {
    if (!cabeça || !cauda) return; // Garante que cabeça e cauda são válidos
    ...
    if (novo == NULL) { // Verificação crucial de malloc
        printf("Erro: Falha na alocação de memória para um novo item.\n");
        return;
    }
    ...
    if (*cabeça == NULL) {
        *cabeça = novo;
        *cauda = novo; // Se a lista estava vazia, novo é cabeça e cauda
    } else {
        (*cauda)->proximo = novo; // Adiciona no final em O(1)
        *cauda = novo;            // Atualiza o ponteiro da cauda
    }
...
void menu_mochila() {
    ...
    No* lista_cauda = NULL; // Novo ponteiro para a cauda da lista
        ...
        switch(opcao) {
            case 1:
                if (estrutura == 0) {
                    ...
                    inserirItemLista(&lista_cabeca, &lista_cauda); // Passa a cauda
                }
                break;
            ...
            case 0:
                ...
                lista_cauda = NULL; // Garante que a cauda também seja resetada após liberar
                return;
